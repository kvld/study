package ru.ifmo.ctddev.kiryukhin.concurrent;
...
public class IterativeParallelism implements ListIP {
    ...
    private <T, R> R doItParallel(int threadsCount, List<? extends T> list, Function<List<? extends T>, R> operation, Function<? super List<R>, R> result) throws InterruptedException {
        ...
        List<ParallelOperation<T, R>> operations = new ArrayList<>(splitList.size());
        splitList.forEach(objects -> operations.add(new ParallelOperation<>(objects, operation)));
        ...
        operations.forEach(op -> operationResults.add(op.getResult()));

        return result.apply(operationResults);
    }

    private class ParallelOperation<T, R> implements Runnable {
        private R result;
        private Function<List<? extends T>, R> operation;
        private List<? extends T> list;

        public ParallelOperation(List<? extends T> list, Function<List<? extends T>, R> operation) {
